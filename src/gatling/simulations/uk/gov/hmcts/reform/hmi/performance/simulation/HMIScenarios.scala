...
import uk.gov.hmcts.reform.hmi.performance.scenarios.{OAuth, RequestHearing}
...
setUp(
  ...
  RHU.inject(nothingFor(120), rampUsers(466) during (3600)),
  RHUC.inject(nothingFor(240), rampUsers(156) during (3600))
) .protocols(httpProtocol).maxDuration(4000)
